...
<form class="form-horizontal" id="account_form" method="POST" action="{{ route('profile.main.update') }}">
	...
	<div class="form-group form-group-sm">
		...
		<div class="col-sm-10">
			...
			@else
			<p  style="color: #333;">{{ Lang::get('messages.profile.account.school_link_actions.linked_with') }} <strong>{{ $user['schoolEmail'] }}</strong> <a class="btn btn-sm btn-danger" href="{{ route('unlinkGoogle') }}">{{ Lang::get('messages.profile.account.school_link_actions.unlink_now') }}</a></p>
			@endif
